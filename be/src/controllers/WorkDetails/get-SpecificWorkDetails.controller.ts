...
export const getSpecificWorkDetailController: RequestHandler = async (
  req,
  res
) => {
  try {
    ...
    const taskers = await taskerModel.find({ location }).populate({
      path: 'workDetails',
    ...
    const hourlyRates = taskers
      ...
      .map((workDetail: any) => Number(workDetail.hourlyRate)) // Convert to number
